...
try:
    ...
    print('Mapping configuration not found')
    print('Have you tried running install.sh?')
    exit(2)
...
def handle_title_change(title):
    ...
    if not os.path.exists(mappings.DEVICE_PATH):
        return

    print(title)

    for i in range(len(mappings.TITLE_MAP)):
        if mappings.TITLE_MAP[i][0].match(title):
...
def remap_keys(new_mapping):
    ...
    if contents:
        current_mapping_process = subprocess.Popen(['actkbd -g -d %s' % mappings.DEVICE_PATH],
